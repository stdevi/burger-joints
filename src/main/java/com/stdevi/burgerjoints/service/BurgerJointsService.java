...
public class BurgerJointsService implements VenueService {
    ...
    @Override
    public Flux<Venue> getBurgerJointsNear(String place) {
        return getVenuesNear(place).flatMap(this::getPhotoUrlsPerVenue).flatMap(this::getVenuesWithBurgerPhoto);
    }

    private Mono<Venue> getVenuesWithBurgerPhoto(Tuple2<Venue, List<String>> venuePhotoUrls) {
    ...
    private Mono<Tuple2<Venue, List<String>>> getPhotoUrlsPerVenue(Venue venue) {
    ...
    private Flux<Venue> getVenuesNear(String place) {
